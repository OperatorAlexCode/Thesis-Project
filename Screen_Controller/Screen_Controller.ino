...
BLEByteCharacteristic SetTileDiscovered(ScreenController.uuid(), BLERead | BLEWrite);
...
void setup() {
  ...
  SetTileDiscovered.setEventHandler(BLEWritten, SetTileDiscoveredEvent);

  ScreenController.addCharacteristic(SetTileRoom);
  ScreenController.addCharacteristic(SetTileState);
  ScreenController.addCharacteristic(SetTileDiscovered);
...
void SetTileRoomEvent(BLEDevice central, BLECharacteristic characteristic) {
  ...
  {
    //SetTile(index, (Room)value, RoomStates[index]);
    RoomSprites[index] = (Room)value;
    UpdateRoom(index);
  }
}
...
void SetTileStateEvent(BLEDevice central, BLECharacteristic characteristic) {
  ...
  {
    //SetState(index,(RoomState)value);
    RoomStates[index] = (RoomState)value;
    UpdateRoom(index);
  }
}

void SetTileDiscoveredEvent(BLEDevice central, BLECharacteristic characteristic) {
  Serial.print("Recieved value:");
  for (int x = 0; x < 8; x++)
    Serial.print(bitRead(SetTileState.value(),x));

  Serial.println(" | Set Discovered");

  byte value /*= 0b00001111 & SetTileRoom*/;
  byte index /*= (0b11110000 & SetTileRoom) >> 4*/;

  GetValues(SetTileDiscovered.value(),value,index);
  ...
  if (index < 16 && Discovered[index] != (bool)value)
  {
    //SetState(index,(RoomState)value);
    Discovered[index] = (bool)value;
    UpdateRoom(index);
  }
}
...
void UpdateRoom(int tile) {
  if (tile >= 8)
    Multi2.openChannel(tile%8);

  else
    Multi.openChannel(tile%8);

  Screen.clearBuffer();

  if (Discovered[tile])
  switch(RoomSprites[tile]) 
  {
    case Room::Bunks:
    ...
    case Room::Storage:
      ...
      break;
    default:
      String output = String("Screen ") + String(tile);
      Screen.drawStr(0, 10, output.c_str());
      break;
  }

  switch (RoomStates[tile] ) {
  ...
  if (tile >= 8)
    Multi2.closeChannel(tile%8);

  else
    Multi.closeChannel(tile%8);
}
...
void SetRoom(int tileToSet, Room room) {
  if (tileToSet >= 8)
    Multi2.openChannel(tileToSet%8);
...
void SetState(int tileToSet, RoomState state) {
  if (tileToSet >= 8)
    Multi2.openChannel(tileToSet%8);
